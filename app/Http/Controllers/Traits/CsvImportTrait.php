...
//dd($for_insert);
            foreach ($for_insert as $insert_item) {
                $categoriesIds = [];
                foreach ($insert_item as $dataBankItem){
$data=[];
                    $dataBank = DataBank::where('email',$dataBankItem['email'])->first();
                    $categoriesIds = $dataBankItem['data_bank_category_id'];
                    if (!is_int($categoriesIds)) {
                        $categoriesIds = explode(",", $categoriesIds);
                    }
                    if(!$dataBank){
                        $unsubscribe_link = Str::random(30);
                        $data['unsubscribe_link'] = $unsubscribe_link;
                        $data['email'] = $dataBankItem['email'];
                        $data['is_subscribe'] = $dataBankItem['is_subscribe'];
                        $data['name'] = $dataBankItem['name'];
                        $dataBank = $model::create($data);
                        $dataBank->data_bank_categories()->sync($categoriesIds,[]);
                    }else{
                        $dataBank->data_bank_categories()->sync($categoriesIds,[]);
                    }
