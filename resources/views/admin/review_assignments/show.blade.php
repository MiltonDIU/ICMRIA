    ...
    <div class="card-body">
        ...
        @else
            <table class="table table-sm mb-0">
                <thead>
                    <tr><th>Reviewer</th><th>How</th><th>Match</th><th>Status</th><th>Assigned</th><th></th></tr>
                </thead>
                <tbody>
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <form action="{{ route('admin.review-assignments.auto', $paper->id) }}" method="POST" class="mb-0">
            ...
            <button class="btn btn-sm btn-success" {{ $eligible === 0 ? 'disabled' : '' }}>
                <i class="fas fa-magic"></i> Assign the best {{ max($wanted - $paper->reviewerAssignments->count(), 0) }} automatically
            </button>
        </form>
    </div>
    ...
    <div class="card-body">
        ...
        @else
            ...
            <form action="{{ route('admin.review-assignments.store', $paper->id) }}" method="POST">
                ...
                <small class="form-text text-muted">
                    Everyone in this track's pool is listed.
                    @if($blocked)
