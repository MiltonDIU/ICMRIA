    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.committees.update', [$committee->id]) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('committee_type_id') ? 'is-invalid' : '' }}" name="committee_type_id" id="committee_type_id" required>
                    @foreach($committeeTypes as $id => $name)
                        <option value="{{ $id }}" {{ (old('committee_type_id') ? old('committee_type_id') : $committee->committee_type_id ?? '') == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
                    ...
                    @foreach($parentCommittees as $groupName => $options)
                        <optgroup label="{{ $groupName }}">
                            @foreach($options as $id => $name)
                                <option value="{{ $id }}" {{ (old('parent_id') ? old('parent_id') : $committee->parent_id ?? '') == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </optgroup>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $committee->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('section') ? 'is-invalid' : '' }}" type="text" name="section" id="section" value="{{ old('section', $committee->section) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $committee->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sort_order') ? 'is-invalid' : '' }}" type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $committee->sort_order) }}">
                ...
                <table class="table table-bordered" id="members-table">
                    ...
                    <tbody>
                        ...
                        @if(old('members'))
                            ...
                            @endforeach
                        @else
                            @foreach($committee->members as $index => $associatedMember)
                                <tr data-index="{{ $index }}">
                                    <td>
                                        <select name="members[{{ $index }}][id]" class="form-control select2" required>
                                            <option value="">-- Select Member --</option>
                                            @foreach($members as $member)
                                                <option value="{{ $member->id }}" {{ $associatedMember->id == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td><input type="text" name="members[{{ $index }}][role]" class="form-control" value="{{ $associatedMember->pivot->role }}"></td>
                                    <td><input type="text" name="members[{{ $index }}][level]" class="form-control" value="{{ $associatedMember->pivot->level }}"></td>
                                    <td><input type="number" name="members[{{ $index }}][sort_order]" class="form-control" value="{{ $associatedMember->pivot->sort_order ?? 0 }}"></td>
                                    <td><button type="button" class="btn btn-danger remove-member">Remove</button></td>
                                </tr>
                                @php $member_index = $index + 1; @endphp
                            @endforeach
