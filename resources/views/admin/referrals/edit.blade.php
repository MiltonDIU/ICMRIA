...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.referral.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.referrals.update", [$referral->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required" for="name">{{ trans('cruds.referral.fields.name') }}</label>
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $referral->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="email" id="email" value="{{ old('email', $referral->email) }}" required>
                    @if($errors->has('email'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('coupon_id') ? 'is-invalid' : '' }}" name="coupon_id" id="coupon_id">


                            @foreach($coupons as $id => $entry)
                                <option value="{{ $id }}" {{ (old('coupon_id') ? old('coupon_id') : $referral->coupon->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active" required>
                        ...
                        @foreach(App\Models\Referral::IS_ACTIVE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_active', $referral->is_active) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
