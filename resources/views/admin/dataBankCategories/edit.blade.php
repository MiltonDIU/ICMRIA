...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.dataBankCategory.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.data-bank-categories.update", [$dataBankCategory->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required" for="title_of_data_bank">{{ trans('cruds.dataBankCategory.fields.title_of_data_bank') }}</label>
                    <input class="form-control {{ $errors->has('title_of_data_bank') ? 'is-invalid' : '' }}" type="text" name="title_of_data_bank" id="title_of_data_bank" value="{{ old('title_of_data_bank', $dataBankCategory->title_of_data_bank) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active" required>
                        ...
                        @foreach(App\Models\DataBankCategory::IS_ACTIVE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_active', $dataBankCategory->is_active) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
