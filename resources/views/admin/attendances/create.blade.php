        ...
        <div class="card-body">
            <form action="{{ route("admin.schedules.store") }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_workshop') ? 'is-invalid' : '' }}" name="is_workshop" id="is_workshop">
                        ...
                        @foreach(App\Models\Schedule::IS_WORKSHOP_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_workshop', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
