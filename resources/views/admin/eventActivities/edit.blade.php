...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.eventActivity.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.event-activities.update", [$eventActivity->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label for="title">{{ trans('cruds.eventActivity.fields.title') }}</label>
                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $eventActivity->title) }}">
                    @if($errors->has('title'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $eventActivity->link) }}">
                    @if($errors->has('link'))
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('summary') ? 'is-invalid' : '' }}" name="summary" id="summary">{{ old('summary', $eventActivity->summary) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active" required>
                        ...
                        @foreach(App\Models\EventActivity::IS_ACTIVE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_active', $eventActivity->is_active) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
