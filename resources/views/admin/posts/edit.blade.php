    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.posts.update", [$post->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="summary">{{ trans('cruds.post.fields.summary') }}</label>
                    <textarea class="form-control {{ $errors->has('summary') ? 'is-invalid' : '' }}" name="summary" id="summary">{{ old('summary', $post->summary) }}</textarea>
                    @if($errors->has('summary'))
                        <span class="text-danger">{{ $errors->first('summary') }}</span>
                    @endif
                    <span class="help-block">{{ trans('cruds.post.fields.summary_helper') }}</span>
                </div>
                <div class="form-group">
                    <label for="details">{{ trans('cruds.post.fields.details') }}</label>
                    <textarea class="form-control ckeditor2 {{ $errors->has('details') ? 'is-invalid' : '' }}" name="details" id="details">{!! old('details', $post->details) !!}</textarea>
                    @if($errors->has('details'))
                        <span class="text-danger">{{ $errors->first('details') }}</span>
                    @endif
                    <span class="help-block">{{ trans('cruds.post.fields.details_helper') }}</span>
                </div>
{{--                <div class="form-group">--}}
...
{{--                </div>--}}
                <div class="form-group">
                    <label class="required" for="blog_category_id">{{ trans('cruds.post.fields.blog_category') }}</label>
                    <select class="form-control select2 {{ $errors->has('blog_category') ? 'is-invalid' : '' }}" name="blog_category_id" id="blog_category_id" required>
                        @foreach($blog_categories as $id => $entry)
                            <option value="{{ $id }}" {{ (old('blog_category_id') ? old('blog_category_id') : $post->blog_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('blog_category'))
                        <span class="text-danger">{{ $errors->first('blog_category') }}</span>
                    @endif
                    <span class="help-block">{{ trans('cruds.post.fields.blog_category_helper') }}</span>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label>{{ trans('cruds.post.fields.is_active') }}</label>
                    <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active">
                        <option value disabled {{ old('is_active', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                        @foreach(App\Models\Post::IS_ACTIVE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_active', $post->is_active) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('is_active'))
                        <span class="text-danger">{{ $errors->first('is_active') }}</span>
                    @endif
                    <span class="help-block">{{ trans('cruds.post.fields.is_active_helper') }}</span>
                </div>
                <div class="form-group">
