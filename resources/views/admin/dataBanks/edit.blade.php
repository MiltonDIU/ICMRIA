...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.dataBank.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.data-banks.update", [$dataBank->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label class="required" for="email">{{ trans('cruds.dataBank.fields.email') }}</label>
                    <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $dataBank->email) }}" required>
                    @if($errors->has('email'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_subscribe') ? 'is-invalid' : '' }}" name="is_subscribe" id="is_subscribe" required>
                        ...
                        @foreach(App\Models\DataBank::IS_SUBSCRIBE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_subscribe', $dataBank->is_subscribe) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $dataBank->name) }}">
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('unsubscribe_link') ? 'is-invalid' : '' }}" type="text" name="unsubscribe_link" id="unsubscribe_link" value="{{ old('unsubscribe_link', $dataBank->unsubscribe_link) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('data_bank_categories') ? 'is-invalid' : '' }}" name="data_bank_categories[]" id="data_bank_categories" multiple required>
                        @foreach($data_bank_categories as $id => $data_bank_category)
                            <option value="{{ $id }}" {{ (in_array($id, old('data_bank_categories', [])) || $dataBank->data_bank_categories->contains($id)) ? 'selected' : '' }}>{{ $data_bank_category }}</option>
                        @endforeach
                    </select>
